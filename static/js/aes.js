...
(function() {
	for (var u = CryptoJS,
			p = u.lib.BlockCipher,
			d = u.algo,
			l = [], s = [], t = [], r = [], w = [], v = [], b = [], x = [], q = [], n = [], a = [], c = 0; 256 > c; c++) a[c] =
		128 > c ? c << 1 : c << 1 ^ 283;
	for (var e = 0,
			j = 0,
			c = 0; 256 > c; c++) {
		...
		d = d.AES = p.extend({
			_doReset: function() {
				for (var a = this._key,
						c = a.words,
						d = a.sigBytes / 4,
						a = 4 * ((this._nRounds = d + 6) + 1), e = this._keySchedule = [], j = 0; j < a; j++)
					if (j < d) e[j] = c[j];
					else {
						var k = e[j - 1];
...
function pwd(data, decrypt_key, decrypt_iv) {
	var key = CryptoJS.enc.Utf8.parse(decrypt_key);
	var iv = CryptoJS.enc.Utf8.parse(decrypt_iv);
	//let encryptedHexStr = CryptoJS.enc.Hex.parse(data);
	//var word = CryptoJS.enc.Base64.stringify(data);
...
function b64(url) {
	var src =  CryptoJS.enc.Base64.stringify(url)
